...
## SQLAlchemy
用pip安装SQLAlchemy
建立引擎from sqlalchemy import create_engine  
with engine.connect() as conn: with语句获取连接  
Connection.commit() 进行块内调用  
Engine.begin() 预先块  
发送参数：使用“:”  
Session.execute() 使用ORM Session  
访问行：元素赋值、整数索引、映射访问  
### 使用数据库源数据
要开始使用 SQLAlchemy 表达式语言，我们需要构造表示所有数据库的对象  
直接构造函数or间接使用 ORM 映射类  
使用 ORM 声明性表单定义表元数据  
使用 ORM 时，集合仍然存在， 然而，它本身与通常称为 ORM 的仅结构相关联 作为声明性基础  
运行 PRAGMA 语句从 ORM 映射向数据库发出 DDL  
表格反射。表反射是指 通过读取电流生成和相关对象数据库的状态  
### 使用数据
#### 使用insert语句
...
使用支持的后端的 RETURNING 子句 自动检索最后插入的主键值 以及服务器默认值的值  
insert into ...  select from ... 从...插入  
#### 使用select语句
select().where表达式构造  
FROM子句通常是根据我们在列中设置的表达式来推断  
如果连接的左侧和右侧目标没有这样的约束，或者 有多个约束，我们需要指定 ON 子句  
